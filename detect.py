...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        if label=='human':
                          humman_counts=humman_counts+1
                        if label=='ice cream':
                          Scoop_counts=Scoop_counts+1
                        # print(label)
                        plot_one_box(xyxy, img_name, im0, label=label, color=colors[int(cls)], line_thickness=2) 
        ...
        count_data.append(img_name)
        count_data.append(humman_counts)
        count_data.append(Scoop_counts)
        if not os.path.exists('Counts.csv'):
          header=['Img_Name','Hummans','Ice_creams']
